...
See [directory layout examples](directory-layouts.md) for universal, small project, and library layouts, plus common mistakes.
...
For application configuration with Cobra + Viper, see [config reference](config.md).

## Tests, Benchmarks, and Examples

Co-locate `_test.go` files with the code they test. Use `testdata/` for fixtures. See [testing layout](testing-layout.md) for file naming, placement, and organization details.

## Go Workspaces

Use `go.work` when developing multiple related modules in a monorepo. See [workspaces](AI/skills/golang-project-layout/references/workspaces.md) for setup, structure, and commands.
