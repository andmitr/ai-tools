...
Use pprof CPU profile to capture a 30s sample (see [pprof.md](AI/skills/golang-troubleshooting/references/pprof.md) for commands), then inspect with `top`, `web`, or `list funcName`.
...
Use pprof heap profile (see [pprof.md](AI/skills/golang-troubleshooting/references/pprof.md)). Compare heap snapshots over time with `go tool pprof -base heap1.prof heap2.prof` to find growth. Use escape analysis (see [diagnostic-tools.md](diagnostic-tools.md)) to find unexpected heap allocations in hot paths.
...
Then use pprof mutex and block profiles (see [pprof.md](AI/skills/golang-troubleshooting/references/pprof.md)).
