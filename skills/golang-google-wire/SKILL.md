...
See [advanced.md](AI/skills/golang-google-wire/references/advanced.md) for the `wire:"-"` exclusion tag and `wire.FieldsOf` details.
...
Wire generates `wire_gen.go` (plain Go, committed, DO NOT EDIT). For a full example with per-package sets, cleanup-heavy graphs, and generated output, see [recipes.md](AI/skills/golang-google-wire/references/recipes.md).
...
Wire generates plain Go constructors, so unit tests use manual injection — no container to clone or reset. For testing patterns (test injectors swapping real providers for fakes, CI stale-check for `wire_gen.go`), see [testing.md](AI/skills/golang-google-wire/references/testing.md).

## Further Reading

- [advanced.md](AI/skills/golang-google-wire/references/advanced.md) — cleanup chains, multiple injectors, set nesting, error catalogue, codegen flags, quick reference
- [recipes.md](AI/skills/golang-google-wire/references/recipes.md) — HTTP server, multi-injector build, cleanup-heavy graph, CLI embedding
- [testing.md](AI/skills/golang-google-wire/references/testing.md) — test injectors, fake bindings, CI stale check
