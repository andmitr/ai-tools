...
`.github/workflows/test.yml` — see [test.yml](test.yml)
...
CI SHOULD enforce code coverage thresholds. Configure thresholds in `codecov.yml` at the repo root — see [codecov.yml](codecov.yml)

---

## Integration Tests

`.github/workflows/integration.yml` — see [integration.yml](integration.yml)
...
`golangci-lint` MUST be run in CI on every PR. `.github/workflows/lint.yml` — see [lint.yml](lint.yml)
...
`.github/workflows/security.yml` — see [security.yml](security.yml)
...
Create `.github/codeql/codeql-config.yml` to use the extended security query suite — see [codeql-config.yml](codeql-config.yml)
...
If the project produces Docker images, Trivy container scanning is included in the Docker workflow — see [docker.yml](docker.yml)
...
`.github/dependabot.yml` — see [dependabot.yml](dependabot.yml)
...
`.github/workflows/dependabot-auto-merge.yml` — see [dependabot-auto-merge.yml](dependabot-auto-merge.yml)
...
Install the [Renovate GitHub App](https://github.com/apps/renovate), then create `renovate.json` at the repo root — see [renovate.json](renovate.json)
...
`.github/workflows/release.yml` — see [release.yml](release.yml)
...
`.goreleaser.yml` — see [goreleaser-cli.yml](goreleaser-cli.yml)
...
`.goreleaser.yml` — see [goreleaser-lib.yml](goreleaser-lib.yml)
...
`.goreleaser.yml` — see [goreleaser-monorepo.yml](goreleaser-monorepo.yml)
...
`.github/workflows/docker.yml` — see [docker.yml](docker.yml)
...
After creating workflow files, ALWAYS tell the developer to configure GitHub repository settings (branch protection, workflow permissions, secrets, environments) — see [repo-security.md](repo-security.md)
...
`.github/workflows/ai-review.yml` — see [claude-code-review.yml](claude-code-review.yml)
...
Copy skills into your repo, then append [copilot-review-instructions.md](copilot-review-instructions.md) to `.github/copilot-instructions.md`:
